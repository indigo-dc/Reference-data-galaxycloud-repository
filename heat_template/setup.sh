...
if [[ -r /etc/os-release ]]; then
    ...
    if [ "$ID" = "ubuntu" ]; then
        ...
        apt-get -y install ansible git vim python-pycurl &>> $LOGFILE
    else
...
#---
# Install cvmfs-server role
ansible-galaxy install mtangaro.cvmfs-server &>> $LOGFILE

# Run cvmfs-role
#ansible-playbook /etc/ansible/roles/mtangaro.cvmfs-server/cvmfs_server.yml &>> $LOGFILE 

# Git clone refdata repo
git clone https://github.com/mtangaro/galaxy-reference-data-repository.git /home/ubuntu/galaxy-reference-data-repository
chown ubuntu:ubuntu /home/ubuntu/galaxy-reference-data-repository
